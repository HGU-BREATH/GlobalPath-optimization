...
def main(file1, file2, file3, file4):
    ...
    plt.plot(x1, y1, 'o-', label='Trajectory1', markersize=3)  # 투명도 설정하여 겹치는 부분을 줄임
    plt.plot(x2, y2, 'o-', label='Trajectory2', markersize=3)
    plt.plot(x3, y3, 'o-', label='Trajectory3', markersize=3)
    plt.plot(x4, y4, 'o-', label='Trajectory4', markersize=3)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Usage: python3 viewTrajectories.py file1.csv file2.csv file3.csv file4.csv")
        sys.exit(1)
