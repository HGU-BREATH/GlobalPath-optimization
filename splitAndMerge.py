...
def find_closest_points(inflection_points, trajectory):
    ...
    _, indices = tree.query(inflection_points[['x', 'y']].values)
...
def main(inflection_points_file, trajectory_dir, waypoint_dir):
    ...
    output_dir = './src/output_segments/'
    ...
    output_dir = '/home/onebean/GlobalPath-optimization/src/optWaypoint'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_file = os.path.join(output_dir, f"{os.path.basename(inflection_points_file)}")
    optWaypointDf.to_csv(output_file, index=False)
